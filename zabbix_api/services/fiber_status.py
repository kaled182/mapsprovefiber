...
def get_oper_status_from_port(port):
    """Obt?m status operacional usando preferencialmente a chave (zabbix_item_key) da porta.
    L?gica:
      1. Se existir port.zabbix_item_key: item.get filtrando por hostid + search key.
         - Interpreta lastvalue (1=up, 2=down) -> retorna se n?o for unknown.
      2. Se resultado 'unknown' OU n?o encontrou item, e houver port.zabbix_itemid: item.get direto por itemids.
         - Interpreta lastvalue.
    Retorna tuple: (status, raw_value, meta_dict)
    """
    from zabbix_api.services.zabbix_service import zabbix_request
    ...
    if key and hostid:
        ...
        if items:
            ...
            if status != 'unknown':
                return status, raw, {'method': 'item_key', 'key': key, 'itemid': it.get('itemid')}
            # Continua para fallback se unknown
            key_itemid = it.get('itemid')
            if not itemid and key_itemid:
                # Usa itemid descoberto como fallback
                itemid = key_itemid
                # prossegue para etapa de itemid

    # 2. Fallback via itemid
    if itemid:
        by_id = zabbix_request('item.get', {
            'output': ['itemid', 'key_', 'lastvalue', 'value_type', 'name'],
            'itemids': [itemid],
            'limit': 1
        }) or []
        if by_id:
            ...
            raw = it.get('lastvalue')
            status = _interpret(raw)
            return status, raw, {'method': 'itemid', 'itemid': itemid, 'key': it.get('key_')}

    return 'unknown', None, {'method': 'not_found_or_unknown', 'key': key, 'itemid': itemid}
